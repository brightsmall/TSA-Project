...
def extract_left_side(image_data):
    ...
    image_strip = image_data[:,160:440,0]
    
    for i in range(1,16):
        image_strip = np.concatenate((image_strip,image_data[:,160:440,i]),axis=0)       
    return image_strip
...
def extract_mirror_side(image_data):
    ...
    image_strip = mirror_image[:,160:440,0]     
    for i in range(1,16):
        image_strip = np.concatenate((image_strip,mirror_image[:,160:440,i]),axis=0)
    return image_strip
...
raw_df['Id'] = new_id
raw_df['Zone'] = zone
zone7_labels = raw_df[raw_df['Zone']=='Zone7']
zone7_labels.reset_index(inplace=True, drop=True)
zone6_labels = raw_df[raw_df['Zone']=='Zone6']
zone6_labels.reset_index(inplace=True, drop=True)
...
pos_folder =  "D:\\TSA\\stage 1\\aps\\Zone 7\\pos\\"
neg_folder =  "D:\\TSA\\stage 1\\aps\\Zone 7\\neg\\"
test_folder = "D:\\TSA\\stage 1\\aps\\Zone 7\\test\\"
...
for i,item in enumerate(id_list):
    ...
    if zone7_labels['Id'].str.contains(item).any():
        label = int(zone7_labels[zone7_labels['Id']==item]['Probability'])
        if label == 1:
...
for i,item in enumerate(id_list):
    ...
    if zone6_labels['Id'].str.contains(item).any():
        label = int(zone6_labels[zone6_labels['Id']==item]['Probability'])
        if label == 1:
