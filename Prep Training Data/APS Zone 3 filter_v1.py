...
def extract_left_side(image_data):
    ...
    image_strip = image_data[:,380:660,0]
    
    for i in range(1,16):
        image_strip = np.concatenate((image_strip,image_data[:,380:660,i]),axis=0)       
    return image_strip
...
def extract_mirror_side(image_data):
    ...
    image_strip = mirror_image[:,380:660,0]     
    for i in range(1,16):
        image_strip = np.concatenate((image_strip,mirror_image[:,380:660,i]),axis=0)
    return image_strip
...
raw_df = pd.read_csv('D:\\TSA\\stage 1\\stage1_labels.csv')
split_labels = raw_df['Id'].str.split('_')
new_id = [x[0] for x in split_labels]
zone = [x[1] for x in split_labels]
raw_df['Id'] = new_id
raw_df['Zone'] = zone
zone3_labels = raw_df[raw_df['Zone']=='Zone3']
zone3_labels.reset_index(inplace=True, drop=True)
zone1_labels = raw_df[raw_df['Zone']=='Zone1']
zone1_labels.reset_index(inplace=True, drop=True)
...
pos_folder =  "D:\\TSA\\stage 1\\aps\\Zone 3\\pos\\"
neg_folder =  "D:\\TSA\\stage 1\\aps\\Zone 3\\neg\\"
test_folder = "D:\\TSA\\stage 1\\aps\\Zone 3\\test\\"
...
for i,item in enumerate(id_list):
    ...
    if zone3_labels['Id'].str.contains(item).any():
        label = int(zone3_labels[zone3_labels['Id']==item]['Probability'])
        if label == 1:
...
test_folder = "D:\\TSA\\stage 1\\aps\\Zone 1\\test_mirror\\"
...
for i,item in enumerate(id_list):
    ...
    if zone1_labels['Id'].str.contains(item).any():
        label = int(zone1_labels[zone1_labels['Id']==item]['Probability'])
        if label == 1:
