...
def extract_left_side(image_data):
    ...
    image_strip = image_data[:,0:280,0]
    
    for i in range(1,16):
        image_strip = np.concatenate((image_strip,image_data[:,0:280,i]),axis=0)       
    return image_strip
...
def extract_mirror_side(image_data):
    ...
    image_strip = mirror_image[:,0:280,0]     
    for i in range(1,16):
        image_strip = np.concatenate((image_strip,mirror_image[:,0:280,i]),axis=0)
    return image_strip
...
raw_df['Id'] = new_id
raw_df['Zone'] = zone
zone14_labels = raw_df[raw_df['Zone']=='Zone14']
zone14_labels.reset_index(inplace=True, drop=True)
zone13_labels = raw_df[raw_df['Zone']=='Zone13']
zone13_labels.reset_index(inplace=True, drop=True)
...
pos_folder =  "D:\\TSA\\stage 1\\aps\\Zone 14\\pos\\"
neg_folder =  "D:\\TSA\\stage 1\\aps\\Zone 14\\neg\\"
test_folder = "D:\\TSA\\stage 1\\aps\\Zone 14\\test\\"
...
for i,item in enumerate(id_list):
    ...
    if zone14_labels['Id'].str.contains(item).any():
        label = int(zone14_labels[zone14_labels['Id']==item]['Probability'])
        if label == 1:
...
test_folder = "D:\\TSA\\stage 1\\aps\\Zone 13\\test_mirror\\"
...
for i,item in enumerate(id_list):
    ...
    if zone13_labels['Id'].str.contains(item).any():
        label = int(zone13_labels[zone13_labels['Id']==item]['Probability'])
        if label == 1:
