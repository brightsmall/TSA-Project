...
def extract_left_side(image_data):
    ...
    image_strip = image_data[:,40:320,0]
    
    for i in range(1,16):
        image_strip = np.concatenate((image_strip,image_data[:,40:320,i]),axis=0)
    return image_strip

#left_test_strip = extract_left_leg("D:\\TSA\\stage 1\\aps\\" + id_list[0] +".aps")
...
def extract_mirror_side(image_data):
    ...
    image_strip = mirror_image[:,40:320,0]     
    for i in range(1,16):
        image_strip = np.concatenate((image_strip,mirror_image[:,40:320,i]),axis=0)
    return image_strip
...
raw_df['Id'] = new_id
raw_df['Zone'] = zone
zone12_labels = raw_df[raw_df['Zone']=='Zone12']
zone12_labels.reset_index(inplace=True, drop=True)
zone11_labels = raw_df[raw_df['Zone']=='Zone11']
zone11_labels.reset_index(inplace=True, drop=True)
...
pos_folder =  "D:\\TSA\\stage 1\\aps\\Zone 12\\pos\\"
neg_folder =  "D:\\TSA\\stage 1\\aps\\Zone 12\\neg\\"
test_folder = "D:\\TSA\\stage 1\\aps\\Zone 12\\test\\"
...
for i,item in enumerate(id_list):
    ...
    if zone12_labels['Id'].str.contains(item).any():
        label = int(zone12_labels[zone12_labels['Id']==item]['Probability'])
        if label == 1:
...
test_folder = "D:\\TSA\\stage 1\\aps\\Zone 11\\test_mirror\\"
...
for i,item in enumerate(id_list):
    ...
    if zone11_labels['Id'].str.contains(item).any():
        label = int(zone11_labels[zone11_labels['Id']==item]['Probability'])
        if label == 1:
