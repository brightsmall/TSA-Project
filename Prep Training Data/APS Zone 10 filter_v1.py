...
def extract_left_side(image_data):
    ...
    image_strip = image_data[:,120:400,0]    
    for i in range(1,16):
        image_strip = np.concatenate((image_strip,image_data[:,120:400,i]),axis=0)
...
def extract_mirror_side(image_data):
    ...
    image_strip = mirror_image[:,120:400,0]     
    for i in range(1,16):
        image_strip = np.concatenate((image_strip,mirror_image[:,120:400,i]),axis=0)          
...
raw_df['Id'] = new_id
raw_df['Zone'] = zone
zone10_labels = raw_df[raw_df['Zone']=='Zone10']
zone10_labels.reset_index(inplace=True, drop=True)
zone8_labels = raw_df[raw_df['Zone']=='Zone8']
zone8_labels.reset_index(inplace=True, drop=True)
...
#    raw_data = read_data("D:\\stage 1\\aps\\" + id_list[i] +".aps")
...
pos_folder =  "D:\\TSA\\stage 1\\aps\\Zone 10\\pos\\"
neg_folder =  "D:\\TSA\\stage 1\\aps\\Zone 10\\neg\\"
test_folder = "D:\\TSA\\stage 1\\aps\\Zone 10\\test\\"
...
for i,item in enumerate(id_list):
    ...
    if zone10_labels['Id'].str.contains(item).any():
        label = int(zone10_labels[zone10_labels['Id']==item]['Probability'])
        if label == 1:
...
test_folder = "D:\\TSA\\stage 1\\aps\\Zone 8\\test_mirror\\"
...
for i,item in enumerate(id_list):
    ...
    if zone8_labels['Id'].str.contains(item).any():
        label = int(zone8_labels[zone8_labels['Id']==item]['Probability'])
        if label == 1:
...
left_strip = extract_left_side ("D:\\stage 1\\aps\\" + id_list[2] +".aps")  


img = plt.imshow(np.rot90(left_strip[0:512,:]))



mirror_strip = extract_mirror_side ("D:\\stage 1\\aps\\" + id_list[0] +".aps")

img = plt.imshow(np.rot90(mirror_strip[0:512,:])) 
