...
raw_df['Id'] = new_id
raw_df['Zone'] = zone
zone16_labels = raw_df[raw_df['Zone']=='Zone16']
zone16_labels.reset_index(inplace=True, drop=True)
zone15_labels = raw_df[raw_df['Zone']=='Zone15']
zone15_labels.reset_index(inplace=True, drop=True)
...
pos_folder =  "D:\\TSA\\stage 1\\aps\\Zone 16\\pos\\"
neg_folder =  "D:\\TSA\\stage 1\\aps\\Zone 16\\neg\\"
test_folder = "D:\\TSA\\stage 1\\aps\\Zone 16\\test\\"
...
for i,item in enumerate(id_list):
    ...
    if zone16_labels['Id'].str.contains(item).any():
        label = int(zone16_labels[zone16_labels['Id']==item]['Probability'])
        if label == 1:
...
test_folder = "D:\\TSA\\stage 1\\aps\\Zone 15\\test_mirror\\"
...
for i,item in enumerate(id_list):
    ...
    if zone15_labels['Id'].str.contains(item).any():
        label = int(zone15_labels[zone15_labels['Id']==item]['Probability'])
        if label == 1:
