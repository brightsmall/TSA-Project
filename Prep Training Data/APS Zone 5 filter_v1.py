...
raw_df['Id'] = new_id
raw_df['Zone'] = zone
zone5_labels = raw_df[raw_df['Zone']=='Zone5']
zone5_labels.reset_index(inplace=True, drop=True)
...
for i,item in enumerate(id_list):
    ...
    if zone5_labels['Id'].str.contains(item).any():
        label = int(zone5_labels[zone5_labels['Id']==item]['Probability'])
        if label == 1:
